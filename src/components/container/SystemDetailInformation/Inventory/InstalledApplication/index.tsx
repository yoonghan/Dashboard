...
interface InstalledApplicationState {
  detailIdx: number;
}

interface InstalledApplicationProps extends WithStyles<typeof styles> {
  inventoryData: any;
}

class InstalledApplication extends React.Component<InstalledApplicationProps, InstalledApplicationState> {
  constructor(props:InstalledApplicationProps) {
    super(props);
    this.state = {
      detailIdx: -1
    }
  ...
  displayDiskInfo = (idx:string) => {
    ...
    this.setState(
      produce<InstalledApplicationState>(draft => {
...
export default withStyles(styles)(InstalledApplication);
